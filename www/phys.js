...
var phys = function(phys) {
	...
	phys.makeBox = function(width, height, world, scale, dynamic, friction, density) {	
	...
	phys.makeCircle = function(radius, world, scale, dynamic, friction, density) {	
		if (not(radius)) radius = 100;
		if (not(world) || not(scale)) {console.log("phys.Mouse() - please provide a world and scale"); return;}
		if (not(dynamic)) dynamic = true;
		if (not(friction)) friction = 1;
		if (not(density)) density = 1;
	...
	phys.makeVertex = function(radius, world, scale, dynamic, friction, density) {	
		if (not(radius)) radius = 100;
		if (not(world) || not(scale)) {console.log("phys.Mouse() - please provide a world and scale"); return;}
		if (not(dynamic)) dynamic = true;
		if (not(friction)) friction = 1;
